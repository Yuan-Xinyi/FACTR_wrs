...
class FACTRTeleopFrankaZMQ(FACTRTeleop):
    ...
    def get_leader_arm_external_joint_torque(self):
        ...
        if external_torque is not None:
            self.get_logger().info(f"[RECV] External torque: {np.round(external_torque, 4)}")
            self.obs_franka_torque_pub.publish(create_array_msg(external_torque))
        else:
            self.get_logger().warn("[WARN] No torque data received.")
        return external_torque

    def update_communication(self, leader_arm_pos, leader_gripper_pos):
        self.get_logger().info(f"[SEND] Leader pos: {np.round(leader_arm_pos, 4)}")
        self.get_logger().info(f"[SEND] Gripper pos: {round(leader_gripper_pos, 4)}")

        # send leader arm position as joint position target to the follower Franka arm
        # self.franka_cmd_pub.send_message(leader_arm_pos, leader_gripper_pos)
        leader_full_pos = np.array(list(leader_arm_pos) + [leader_gripper_pos], dtype=np.float64)
        # print(f"[SEND] Leader full pos: {np.round(leader_full_pos, 4)}")
        self.franka_cmd_pub.send_message(leader_full_pos)
        ...
        if franka_state is not None:
            self.get_logger().info(f"[RECV] Franka state: {np.round(franka_state, 4)}")
            self.obs_franka_state_pub.publish(create_array_msg(franka_state))
        else:
            self.get_logger().warn("[WARN] No Franka state received.")
